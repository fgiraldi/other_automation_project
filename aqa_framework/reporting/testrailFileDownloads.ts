                    ...
                    .addResultsForCases(
                        ...
                        cases.map(
                            ({
                                ...
                                return {
                                    case_id,
                                    assignedto_id: this._config.run
                                        .assignedto_id,
                                    comment:
                                        '# Automated Scenarios: #\n' +
                                        scenarios,
    ...
    private addRun = (case_ids: number[]) => {
        ...
        return this._client
            .addRun(this._config.run.project_id, {
                name: `[AUTOMATION] File Downloads - ${this.getDate(
                    new Date()
                )}`,
                suite_id: this._config.run.suite_id,
...
let resultsJson = {};
readdirSync(resolvePath(__dirname, '../tmp/json_results/')).forEach(
    (fileName, index) => {
        resultsJson[index] = require(resolvePath(
            __dirname,
            '../tmp/json_results/' + fileName
        ));
    }
);
